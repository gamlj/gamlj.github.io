...
Before looking at the results of the model estimation, we should remember that the ordinal model estimated by `r modulename()` is a _proportional odds model_, in which we assume that the coefficient associated with an independent variable is the same for all logit estimated along the dependent variable scale (cf \@ref(ordinal)). `r modulename()` provides a test for this assumption, usually named _parallel lines test_. It is named like that because proportional odds functions, when estimated in the logit scale, are equivalent to parallel lines.
