...
To overcome this problem, some software allows specifying the clustering variable as `School:Class`, so classes can be distinguished. `r gamlj()` does not allow that, because there's really no reason to it and it may be confusing. The solution in `r gamlj()` is to recode the classes such that they all have a unique code. In `r jamovi()` one can simply compute a new variable as follows:
