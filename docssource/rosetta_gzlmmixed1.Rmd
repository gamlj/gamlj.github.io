...
Here you can find comparisons of results obtained in `r jamovi()` `r gamlj()`, R, and SPSS. When not explicitly discussed, the code of different software is written with the aim of obtaining identical results across programs (not necessarily with the most efficient strategy).

In this example we compare `r gamlj()` results with R results obtained by IDRE on their [tutorial web page](https://stats.idre.ucla.edu/r/dae/mixed-effects-logistic-regression/), and with SPSS results.
...
`r jamovi()` `r gamlj()` module for `Generalized mixed models` can be used to estimate a random coefficient mixed model. First, it requires choose the specific model we want to run: the `Logistic` model.
...
For the predictors coefficients we have substantially the same results as in R, apart from rounding. For the intercept  we have different results. The reason is the different default coding of the variables in `r gamlj()` and in R. In R default, factors are coded with the _dummy coding scheme_, that is 
...
and continuous independent variables are not scaled. In `r gamlj()` the default coding system for categorical variable is ``r opt("simple")`.
