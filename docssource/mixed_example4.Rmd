...
In multilevel designs there are at least two layers, or levels, in the sample: the within-cluster level and the between-cluster levels. This structure is simply declared in the models by specifying a cluster variable and at least one random coefficients that varies across clusters. In many application of the mixed model, however, there are more than 2 levels. In a classical educational program, one can have pupils (layer within), classes (layer 2) and schools (layer 3). An experiment may have several participants that are exposed to several stimuli, creating two clustering variables. Clusters variables groups may be nested or cross-classified (we can call them crossed for simplicity). Whether the cluster variables groups are nested or crossed make a substantial difference in the model structure, and on the parameters that one can estimate. Therefore, it is important to make clear how different structure of the data can be included in the mixed model. Here we see the possibilites in `r modulename()`.   
...
Two or more clustering variables are said to be crossed when each level (group) of one cluster variable is repeated for all levels (groups) of the other clustering variable. An example can be an experiment in which each participant is exposed to a series of stimuli. All participants see all stimuli. An example  can be found `r link_pages(nickname="subjects_by_stimuli")`, using the `r datafile("subjects_by_stimuli", "https://github.com/gamlj/gamlj.github.io/blob/master/data/subjects_by_stimuli.csv")` dataset. Here we have 50 participants measured in two conditions using 30 stimuli. The first cells of a contingency table `subj` by `stimuli` is shown here. 

`r pic("examples/mixed4/data1_freq.png")`

What is crucial here is that both `subj` and `stimuli` clustering variables are a the same level (or layer). This means that each measurement (each row) belongs to one combination of `subj` and `stimuli`, that is is measured for one particular participant and one particular stimulus. The classifications are __orthogonal_, meaning that they are independent one each other. In the experimental terminology , they create a balanced 50 X 30 design. 
 
To understand the structure of the model estimable in these designs, we can focus on the random intercepts (the same applies to slopes, but the latter depends on the specific design at hand). What random intercepts can we estimate in this kind of designs? The random intercepts varying across participants, each intercept corresponding to the average score of the participant averaged across the 30 stimuli. The random intercepts varying across stimuli, each intercept corresponding to the average score associated with a stimulus, averaged across the 50 participants.
Thus, the number of intercepts across participant will be N, where N is the number of participants, and the number of intercepts across stimuli would be K, where K is the number of stimuli.

If within each combination of `subj` and `stimuli` there are several measurements, one can also estimate the intercepts vary across `subj` by `stimuli` cells. If `subj` and `stimuli` were factors and not clusters, one could say that the random intercepts can represent the variance of the main effect of `subj`, the main effect of `stimuli` and (if there are enough data), their interaction. For this variance, the number of intercepts are N X K.
...
Then one specify the random structure as for any other mixed model:

`r pic("examples/mixed4/data1_random.png")`

The results will show the correct variance in the tables
...
What is I want to insert also a random intercept that varies across the cells representing the combinations of `subj` and `stimuli` (notice that in this dataset it does not make much sense because there are only two scores in each `subj` by `stimuli` combination, but we show this anyway as an example).

There are two ways in `r modulename()` to obtain the model structure we desire. Going by data or going by formula: they give exactly the same results, so which way to go depends on which way is more convenient from a practical point of view.
...
and specify the value of the new variable as a combination of the values of `subj` and `stimuli`

`r pic("examples/mixed4/input1_compute2.png")`

Then we can add `subj_stimuli` as a clustering variable of our model, and ask for the random intercept across it.
...
Some user may find this method tiring. For those, the module can create the combination variable automatically. When more than one cluster variables are defined, the option ``r opt("Crossing by formula")` options appears. By selecting it, we ask the module to list all possible crossing between clustering variables, so we can select the one we need
...
An example can be found in the  `r datafile("subjects_by_stimuli", "https://github.com/gamlj/gamlj.github.io/blob/master/data/subjects_on_stimuli.csv")`
...
Please notice the numerosity of the clusters. There are 600 observations, 600 stimuli and 20 participants. 
