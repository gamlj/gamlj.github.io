...
Consider a regression $y=a+b_{yx.z}x+b_{yz.x}z$. Recall the the $SS_{yx.z}$ ($x$ explains $y$ keeping constant $z$) is computed as $SS_{total}-SS_{yz}$, where $SS_{yz}$ is the sum of squares explained by $z$ without $x$ in the model, and $SS_{yz.x}=SS_{total}-SS_{yx}$. It follows that the   $\sum{SS_f}=2\cdot SS_{total}-SS_{yx}-SS_{yz}$ is larger than $SS_{total}$ when 
